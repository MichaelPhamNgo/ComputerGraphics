    ...
    void createImage() {
        ...
        while (input.hasNext()) {
            ...
            if (command.equals("DIM")){
                ...
                Vector target = new Vector(x2, y2, z2, 1);
        		...
        		lookAt = new Matrix(new Vector(xaxis.x, yaxis.x, zaxis.x, 0), 
        							new Vector(xaxis.y, yaxis.y, zaxis.y, 0), 
        							new Vector(xaxis.z, yaxis.z, zaxis.z, 0), 
        							new Vector(-xaxis.dotProduct(eye), 
        										-yaxis.dotProduct(eye), 
        										-zaxis.dotProduct(eye), 1));
            	...
            	far = input.nextFloat();
            	perspective = new Matrix(new Vector(2/(right - left), 0, 0,  -(right + left)/(right - left)), 
										 new Vector(0, 2/(top - bottom)	, 0, -(top + bottom)/(top - bottom)), 
										 new Vector(0, 0, -2/(far - near), -(far + near)/(far - near)), 
										 new Vector(0,0,0, 1));
            	...
            	near = input.nextFloat();
            	far = input.nextFloat();
            	perspective = new Matrix(new Vector(2 * near / (right - left), 0, 0, -((right + left) * near) / (right - left)), 
						 new Vector(0, 2 * near / (top - bottom), 0, -((top + bottom) * near) / (top - bottom)), 
						 new Vector(0, 0, -(far-near) / (far - near), (2 * far *near) / (near - far)), 
						 new Vector(0,0,-1, 0));
